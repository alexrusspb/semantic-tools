 ...
 * *** FIXME	remove - удаляет только через поиск. Добавить 
 * 		удаление объекта напрямую: remove(myNode);
 * *** FIXME	insert - нужно больше параметров; нет проверки, 
 * 		что узел уже есть и его не надо добавлять
 * 
 * *** FIXME добавить методы неточного поиска 
 * *** FIXME сохранения/чтения сети в/из файла
 * 
 * @author alex
 *
...
public class SemanticNetwork<T extends Node> implements Serializable {
	...
	// FIXME добавить константы рутовых нод для разных локалей в Node
	/* корневой узел сети */
